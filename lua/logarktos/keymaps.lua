...
local function actions()
	return {
		...
		ai_mode             = { "n", function() L().work_mode_tab() end, "WorkMode (three panes from logarktos.lua)" },
		work                = { "n", function() L().work_mode_tab() end, "WorkMode (three panes from logarktos.lua)" },
		here_work           = { "n", function() L().here_work_mode() end, "WorkMode in the current tab" },
		...
		markdown_drafts     = { { "n", "x" }, function()
			local mode = vim.api.nvim_get_mode().mode
			if mode == "v" or mode == "V" then
				MD().markdown_drafts({ line1 = vim.fn.line("v"), line2 = vim.fn.line(".") })
			else
				MD().markdown_drafts()
			end
		end, "Draft current/selected Markdown file(s)" },
